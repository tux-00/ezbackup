...
def cp(path):
	...
	if os.path.isdir(path):
		...
		try:
			...
			_print("Error adding {0} file to tar archive: {1}.".format(path, e.strerror))
...
def ftp_connect(host, port, login, passwd, dir):
	...
	try:
		...
		_print("Error changing FTP directory to {0}: {1}.".format(SAVE_DIR, e.message))
		ftp_quit()
		quit_ezbackup(ERROR_CODE)
...
def mail(sender, receivers, subject, msg):
	try:
		...
		_print("Error sending email to {0} : {1}.".format(receivers, e.strerror))
		return False
...
if something_saved == True:
	with open(tarname, 'rb') as savedtar:
		try:
			ftp.storbinary('STOR ' + tarname, savedtar)
		except Exception as e:
			_print("Error copying {0} to FTP server: {1}.".format(tarname, e.message))
			quit_ezbackup(ERROR_CODE)
